...
https://github.com/clovaai/deep-text-recognition-benchmark/blob/master/dataset.py """
...
class BatchBalancedDataset(ConcatDataset):
    def __init__(self, opt):
        ...
        for selected_d, batch_ratio_d in zip(opt.select_data, opt.batch_ratio):
            ...
            _dataset, _ = torch.utils.data.random_split(_dataset, dataset_split)
            self.dataset_list.append(_dataset)
            self.dataset_len_list.append(len(_dataset))
            
            selected_d_log = f'num total samples of {selected_d}: {total_number_dataset} \
